...
const addData = (id) => {
  router.push({ path: `/editCategories/0` });
};

const edit = (id) => {
  router.push({ path: `/editCategories/${id}` });
};

const _delete = async (id) => {
  try {
    const response = await axios.get(`/api/delete_categories?id=${id}`);
...
const toggleStatus = async (d) => {
  ...
  try {
    await axios.post('/api/update_status', { id: d.id, status: d.status, type: 'categories' });
  } catch (error) {
...
<template>
    <v-container>
    <h2 style="margin-bottom: 2%; margin-top: -2%;">Categories</h2>
    <v-row no-gutters>
      <v-col cols="12" md="3">
